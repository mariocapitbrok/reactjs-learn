...
test('when categoryValue is NOT an array', function () {
  var data = [{
    category: 'Fruits',
    price: 1,
    stocked: true,
  ...
  var result = groupData(data, 'category');
  expect(result).toEqual(sample);
});
test('when categoryValue is an array', function () {
  var coffees = [{
    title: 'Black',
  ...
  var result = groupData(coffees, 'ingredients');
  expect(result).toEqual(sample);
});